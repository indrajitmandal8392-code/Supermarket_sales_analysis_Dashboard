...
Date: September 2026
Environment: Python 3.10+ / Pandas / Seaborn / Matplotlib
================================================================================
"""
...
# 
# ### Objective
# Uncover critical operational and customer behavioral patterns across three supermarket branches 
# (Yangon, Mandalay, Naypyitaw) to optimize branch profitability, inventory turnover, customer loyalty, 
# and peak-hour staffing.
...
# Suppress deprecation and future warnings for clean terminal and notebook output
...
def load_dataset() -> pd.DataFrame:
    """
    Attempts to load local CSV from various standard paths.
    Falls back to high-fidelity mock generator if no file is present.
    """
    candidate_paths = [
        'supermarket_sales.csv',
        'data/supermarket_sales.csv',
        '../data/supermarket_sales.csv',
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'supermarket_sales.csv'),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'supermarket_sales.csv')
    ]
...
print("=" * 88)
print("SECTION 1: BASELINE SANITY CHECK & DATA PROFILING — ANALYST: INDRAJIT MANDAL")
print("=" * 88)
...
if missing_counts.sum() > 0:
    ...
    print("[PASS] Zero missing / null values detected across all columns.")

print("\n--- Duplicate Records Audit ---")
dup_count = raw_df.duplicated().sum()
print(f"[PASS] Duplicate Rows: {dup_count}")

print("\n--- Statistical Profile (Numerical Features) ---")
print(raw_df.describe().T[['count', 'mean', 'std', 'min', '50%', 'max']])

print("\n--- Categorical Cardinality ---")
cat_cols = [c for c in raw_df.columns if raw_df[c].dtype == object or pd.api.types.is_string_dtype(raw_df[c])]
for col in cat_cols:
    print(f"  * {col:<15}: {raw_df[col].nunique():<3} unique values -> {list(raw_df[col].unique()[:5])}")
...
print("\n" + "=" * 88)
print("SECTION 2: DATA CLEANING & PREPROCESSING PIPELINE — ANALYST: INDRAJIT MANDAL")
print("=" * 88)

# 2.1 Standardize Column Headers to snake_case
clean_col_map = {
...
print("[OK] Standardized column names to snake_case.")

# 2.2 Datetime Parsing & Temporal Feature Extraction
df['parsed_date'] = pd.to_datetime(df['date'])
df['parsed_time'] = pd.to_datetime(df['time'], format='%H:%M').dt.time
...
# Categorical day order for consistent sorting
...
print(f"[OK] Financial Logic Sanity Check (Tax 5% == Gross Income): {'PASSED' if tax_tolerance else 'FAILED'}")
print(f"[OK] Accounting Integrity Check (COGS + Tax == Total): {'PASSED' if cogs_plus_tax else 'FAILED'}")
print(f"[OK] Engineered Features Added: 'hour', 'month', 'month_name', 'day_of_week', 'day'.")
...
print("\n" + "=" * 88)
print("SECTION 3: GENERATING HIGH-RESOLUTION VISUALIZATIONS — ANALYST: INDRAJIT MANDAL")
print("=" * 88)
...
plt.tight_layout()
plot1_path = os.path.join(OUTPUT_DIR, "plot1_branch_city_financials.png")
...
plt.tight_layout()
plot2_path = os.path.join(OUTPUT_DIR, "plot2_product_line_performance.png")
...
plt.tight_layout()
plot3_path = os.path.join(OUTPUT_DIR, "plot3_demographic_loyalty_analysis.png")
...
plt.tight_layout()
plot4_path = os.path.join(OUTPUT_DIR, "plot4_sales_velocity_hourly.png")
...
plt.suptitle('Figure 5: Payment Channel Adoption & Channel Spend Distribution\n(Lead Analyst: Indrajit Mandal)', 
             fontsize=15, fontweight='bold', y=1.02)
plt.tight_layout()
plot5_path = os.path.join(OUTPUT_DIR, "plot5_payment_method_preferences.png")
...
plt.tight_layout()
plot6_path = os.path.join(OUTPUT_DIR, "plot6_customer_satisfaction_ratings.png")
...
print("\n--- Detailed Branch Performance Summary ---")
for b, row in branch_summary.iterrows():
    print(f"  * Branch {b} ({row['city']:<10}): Revenue = ${row['revenue']:>9,.2f} | Orders = {row['orders']} | AOV = ${row['aov']:.2f}")

print("\n--- Detailed Product Line Performance Summary ---")
for p, row in product_summary.iterrows():
    print(f"  * {p:<25}: Sales = ${row['revenue']:>9,.2f} | Units = {row['quantity']:>4} | Gross Profit = ${row['income']:>8,.2f} | CSAT = {row['mean_rating']:.2f}/10")

print("\n[SUCCESS] Pipeline execution finished with zero warnings. All charts exported to /visualizations.")
